...
source('./analysis/independentModel.R')
...
bind_rows(objOne.AbAc, objTwo.AbAc, key.AbAc, objOne.BaCa, objTwo.BaCa, key.BaCa, .id = 'type') %>%
  ...
  group_by(subject_id, study_id, condition) %>%
...
left_join(dependancy.df, independent.df, by = c('subject_id', 'study_id', 'condition'), suffix = c('.data', '.indep')) -> final.dependancy
