...
tidy.ret %>%
  ...
  labs(x = 'slider_rt (s)', title = 'Cued Recall', subtitle = 'Submit RT Distributions', y = 'subjects', caption = 'dashes = trials; red dashed = 10 minutes.') +
  facet_grid(~session) +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
```
...
tidy.bds %>%
  ...
  geom_point(shape = '|', size = 1.5) +
  scale_x_continuous(breaks = seq(0,300,50)) +
  labs(x = 'reaction time (s)', title = 'Backwards Digit Span', subtitle = 'RT Distributions', y = 'subjects', caption = 'dashes = trials') +
  facet_grid(~session) +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
```
...
tidy.catch %>%
  ...
  select(subject_id, session, response) %>%
  paged_table()
```
