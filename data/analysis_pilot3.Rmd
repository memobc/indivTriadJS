...
source('sort_associations.R')

df <- read_csv('tidy_ret.csv', show_col_types = FALSE)

df %>%
  unite(col = 'subject', all_of(c('subject', 'day'))) -> df

df.enc <- read_csv('tidy_enc.csv', col_types = 'ddffcccifdddd')

df.enc %>%
  unite(col = 'subject', all_of(c('subject', 'day'))) -> df.enc
  
df.raw <- read_csv('raw.csv', show_col_types = FALSE)
```
...
df %>%
  ...
  pivot_wider(id_cols = all_of(c('encTrialNum', 'subject')), names_from = associationType, values_from = isCorrect) -> personTriads
...
df %>%
  ...
  nest(data = c(starts_with('resp_'), all_of(c('objOne', 'objTwo', 'RetKeyType')))) %>%
  mutate(associationType = map_chr(.x = data, .f = sort_associations)) %>%
  pivot_wider(id_cols = all_of(c('encTrialNum', 'subject')), names_from = associationType, values_from = isCorrect) -> placeTriads
