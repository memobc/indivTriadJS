...
library(ggridges)
library(lme4)
df <- read_csv('tidy_ret.csv', col_types = 'fidfccccccciicccfdiilf')
df.enc <- read_csv('tidy_enc.csv', col_types = 'ficccfdi')
...
ggplot(df.enc, aes(x = rt, y = subject)) +
  ...
  scale_x_continuous(breaks = seq(1000, 15000, 1000))
```
...
```{r}
df %>% 
  unite(col = 'KeyType', RetKeyType, keyType) -> df
...
ggplot(df.summarised, aes(x = KeyType, y = propCorrect)) +
  ...
  stat_summary(geom = 'crossbar', fun.data = mean_se, width = 0.2) +
  labs(title = 'Retrieval Success Does NOT Depend on Type of Memory Cue',
...
df %>%
  ...
  nest(data = all_of(c('resp_opt_1', 'resp_opt_2', 'resp_opt_3', 'resp_opt_4', 'resp_opt_5', 'resp_opt_6', 'objOne', 'objTwo', 'KeyTypeRet'))) %>%
  mutate(associationType = map_chr(.x = data, .f = sort_associations)) %>%
  filter(!(subject == '26612' & encTrialNum == 10)) %>%
  pivot_wider(id_cols = all_of(c('encTrialNum', 'subject')), names_from = associationType, values_from = isCorrect)  %>%
...
dependency <- data.frame(matrix(0, nrow = 10*3, ncol = 3))
names(dependency) <- c("SubID","Pair","Difference")
row <- 0
subjects <- levels(personTriads$subject)
...
names(dependency) <- c("SubID","Pair","Difference")
row <- 0
subjects <- levels(placeTriads$subject)
for (idx in 1:length(subjects)) {
...
```{r}
# plot!
ggplot(dependencySummarised, aes(x = keyType, y = dependency)) +
...
ggplot(tmp.Joined, aes(x = person_memory, y = person_dependency)) +
  geom_point() +
  geom_smooth(formula = y ~ x, method = 'lm', se = F) +
  labs(title = 'People Who Show Little Memory Dependency Are At Performance Ceiling', subtitle = 'Person Triads', y = 'Person Dependency', x = 'Memory Performance on All Person Triad Associations')
```

## What did participants say in the new pilot during catch trials?

```{r, warning=FALSE}
files <- list.files(pattern = 'experiment_data.csv')
df <- map_dfr(files, read_csv, show_col_types = F)
```

```{r}
df %>%
  filter(trial_type == 'survey-text' & phase == 'enc') %>%
  select(subject, encTrialNum, response, where(~!all(is.na(.x))))
```
